...
def detect_A_models(df):
    ...
    # ⛓️ Track longer match signatures to exclude pairs
    longer_signatures = []
    ...
    # 🔍 Detect valid pairs (2 travelers to 0)
    for output in df["Output"].unique():
        ...
        for i in range(len(subset) - 1):
            ...
            sig = (row1["M #"], row2["M #"])
            sig_sorted = tuple(sorted(sig) + [0])
            if any(set(sig_sorted).issubset(set(l)) and len(l) > 2 for l in longer_signatures):
                continue  # 🔁 Already part of longer match
            ...
            if model:
                model_outputs[model + "pr"].append({
                    ...
                    "output": output,
                    "timestamp": last["Arrival"],
                    "sequence": pd.concat([prior, pd.DataFrame([last])]),
                    "feeds": pd.concat([prior, pd.DataFrame([last])])["Feed"].nunique()
                })
