...
st.title("🅰️ Position A Models – Output-Centric Scanner with Pair Detection")
...
def detect_A_models(df):
    ...
    longer_sequences_by_output = defaultdict(list)  # output -> list of sets of M #s
    ...
    for output in df["Output"].unique():
        ...
        for seq in sequences:
            ...
            if seq.shape[0] >= 3:
                longer_sequences_by_output[output_val].append(m_numbers)
                model_id = model
            else:
                embedded = any(m_numbers.issubset(longer) for longer in longer_sequences_by_output[output_val])
                if embedded:
                    continue
                model_id = f"{model}pr"
...
def show_a_model_results(model_outputs, report_time):
    ...
    for base_code, label in labels.items():
        for suffix in ["", "pr"]:
            ...
            label_prefix = "Pair to" if suffix == "pr" else "2+ to"
            header = f"{code}. {label_prefix} {label} – {output_count} output{'s' if output_count != 1 else ''}"
