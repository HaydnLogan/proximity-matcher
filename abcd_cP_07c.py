...
def show_a_model_results(model_outputs, report_time):
    ...
    for code, label in labels.items():
        ...
        today_results = [r for r in results if "Today [0]" in r["sequence"].iloc[-1]["Day"]]
        other_results = [r for r in results if "Today [0]" not in r["sequence"].iloc[-1]["Day"]]
        ...
        def render_result_group(title, group):
            ...
            for out_val, items in output_groups.items():
                ...
                header = f"🔸 Output {out_val:,.3f} – {len(items)} descending {hrs} hours at {ts}"
        ...
        with st.expander(header):
            if today_results: render_result_group("📅 Today", today_results)
            if other_results: render_result_group("📦 Other Days", other_results)
