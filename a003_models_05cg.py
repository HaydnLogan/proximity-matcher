...
def classify_C_model(seq):
    ...
    has_flip = any(signs[i] != signs[i+1] and 0 not in (signs[i], signs[i+1]) for i in range(len(signs)-1))
    if not has_flip: return None, None

    last_time = seq.iloc[-1]["Arrival"]
    if last_time.hour == 18: tcat = "Open"
    elif 18 < last_time.hour < 2 or (last_time.hour == 1 and last_time.minute < 59): tcat = "Early"
    else: tcat = "Late"
    ...
    elif seq.iloc[-1]["Arrival"].hour < 2:
        return "C01", "Polarity Shift After Midnight"
    else:
        return "C01s", "Polarity Shift Before Midnight"
...
def detect_C_models(df, model_outputs, all_signatures):
    for output in df["Output"].unique():
        ...
        for i in range(len(subset) - 1):
            for j in range(i + 2, min(i + 5, len(subset))):
                seq = subset.iloc[i:j + 1]
