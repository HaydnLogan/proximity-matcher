...
# --- Cleanup Step 1: Remove rows with Output <= 0 ---
df = df[df['Output'] > 0]

# --- Cleanup Step 2: Remove rows with unique Output ---
...
# --- Final Cleanup Step: Drop rows with missing Arrival or Output ---
initial_len = len(df)
...
# --- Query 3.1x / 3.2x ---
def query_3x(df, target_day, role_check_fn, label):
    results = []
    day_df = df[df['Day'] == target_day]
    for i, row1 in day_df.iterrows():
        for j, row2 in df[df['Arrival'] < row1['Arrival']].iterrows():
            if row1['Output'] != row2['Output']:
                continue
            m1, m2 = row1['M Name'], row2['M Name']
            if not role_check_fn(m1, m2):
                continue
            if (800 <= row1['Origin'] <= 1300) or (800 <= row2['Origin'] <= 1300):
                results.append({
                    ...
                    'Older Arrival': row2['Arrival'],
                    'M Newer': m1,
                    'M Older': m2,
                    'Output': row1['Output'],
                    'Origin New': row1['Origin'],
                    'Origin Old': row2['Origin'],
                    'Day New': row1['Day'],
                    'Day Old': row2['Day']
                })
    return results
...
query_3_2a = query_3x(df, "Today [0]", lambda x, y: y not in [1, -1] and x not in [1, -1], "Query 3.2a - Today #→# (≠±1)")
query_3_2b = query_3x(df, "Yesterday [1]", lambda x, y: y not in [1, -1] and x not in [1, -1], "Query 3.2b - Yesterday #→# (≠±1)")
    ...
    for i, res in enumerate(results[::-1]):
        ...
        with st.expander(summary):
            df_pair = pd.DataFrame([
                [res['Row Old'], res['Older Arrival'], res['M Older'], res['Origin Old'], res['Day Old']],
                [res['Row New'], res['Newest Arrival'], res['M Newer'], res['Origin New'], res['Day New']]
