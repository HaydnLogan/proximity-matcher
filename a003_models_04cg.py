...
# Project file 3; Models, v6, A & B models
# -----------------------
# Helper functions
# -----------------------
def feed_icon(feed):
...
def detect_A_models(df):
    ...
    for output in df["Output"].unique():
        subset = df[df["Output"] == output].sort_values("Arrival", ascending=True).reset_index(drop=True)
        today_rows = subset[subset["Day"].str.contains("\[0\]")]
        ...
        for i in range(len(subset)):
            ...
            for j in range(i, len(subset)):
                ...
                if len(path) >= 3:
                    seq = subset.loc[path]
                    m_values = seq["M #"].tolist()
                    polarities = [m > 0 for m in m_values[:-1]]
                    all_same = all(p == polarities[0] for p in polarities)
                    today_count = sum("[0]" in d for d in seq["Day"])
                    origin_check = any(o.lower() in epic.union(anchor) for o in seq["Origin"])
                    feeds = seq["Feed"].nunique()

                    if today_count >= 2 and origin_check:
                        sig = sequence_signature(seq)
                        if sig in all_signatures:
                            continue
                        all_signatures.add(sig)
                        label = "B01" if all_same and feeds == 1 else "B02" if not all_same else None
                        if label:
                            model_outputs[label].append({
                                "label": f"{'Same' if label=='B01' else 'Mixed'} Polarity Descent",
                                "output": output,
...
def show_a_model_results(model_outputs, report_time):
    ...
    st.subheader("🔍 A + B Model Results")
