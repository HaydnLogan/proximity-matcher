...
def detect_A_models(df):
    ...
    all_signatures = set()
    near_miss_outputs = defaultdict(list)
    ...
    for output in df["Output"].unique():
        ...
        pairs, near_misses = find_pairs(subset, all_signatures)
        for seq in pairs:
            sig = sequence_signature(seq)
            if sig in all_signatures:
                continue
        ...
        for seq in near_misses:
            ...
            model, label = classify_A_model(last, prior)
            if model:
                near_miss_outputs[model + "pr"].append({
                    "label": f"(Filtered) Pair to {label}",
                    "output": output,
                    "timestamp": last["Arrival"],
                    "sequence": seq,
                    "feeds": seq["Feed"].nunique()
    ...
    return model_outputs, near_miss_outputs, report_time

def show_a_model_results(model_outputs, near_miss_outputs, report_time):
    base_labels = {
    ...
    for code, label in base_labels.items():
        for suffix, title in [("", f"2+ to {label}"), ("pr", f"Pair to {label}"), ("pr_filt", f"(Filtered) Pair to {label}")]:
            key = code + suffix.replace("_filt", "pr")
            results = model_outputs.get(key, []) if "filt" not in suffix else near_miss_outputs.get(key, [])
            if not results:
                continue
            output_count = len(set(r["output"] for r in results))
            header = f"{code}{suffix}. {title} – {output_count} output{'s' if output_count != 1 else ''}"
...
def run_a_model_detection(df):
    model_outputs, near_miss_outputs, report_time = detect_A_models(df)
    show_a_model_results(model_outputs, near_miss_outputs, report_time)
